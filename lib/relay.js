...
function idFetcher(sequelize, nodeTypeMapper) {
  return (() => {
    var _ref = (0, _bluebird.coroutine)(function* (globalId, context) {
      ...
      return model ? sequelize.models[model].findById(id, { requestUser: context && context.user ? context.user : null }) : nodeType ? nodeType.type : null;
    });
...
function createConnectionResolver(_ref2) {
  ...
  let toCursor = function toCursor(item, index) {
    var _getModelOfInstance = getModelOfInstance(item);

    const primaryKeyAttribute = _getModelOfInstance.primaryKeyAttribute;
  ...
  let argsToWhere = function argsToWhere(args) {
    ...
    _lodash2.default.each(args, (value, key) => {
      if (key === "where") {
        _lodash2.default.assign(result, (0, _replaceWhereOperators.replaceWhereOperators)(value));
      }

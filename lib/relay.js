...
function idFetcher(sequelize, nodeTypeMapper) {
  return (() => {
    var _ref = (0, _bluebird.coroutine)(function* (globalId, context, info) {
      ...
      if (model) {
        return sequelize.models[model].findByPk(id, { requestUser: context && context.user ? context.user : null });
      }
...
function createConnectionResolver(_ref2) {
  ...
  _before = _before || (options => {
    options[_dataloaderSequelize.EXPECTED_OPTIONS_KEY] = _dataloaderSequelize.EXPECTED_OPTIONS_KEY;
    return options;
  });
